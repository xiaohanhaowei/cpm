...
import imagewarp
import copy
# TODO: both the `tfr_file` name and `dataset_dir` names need modify
# for train
tfr_file = 'fas_train_dataset.tfrecords'
dataset_dir = '/home/wanghongwei/WorkSpace/datasets/fans/datasets/img/'

# for test
# tfr_file = 'fas_train_dataset.tfrecords'
# dataset_dir = '/home/wanghongwei/WorkSpace/datasets/fans/datasets/img_test/'

# for evaluation
# '''
# tfr_file = 'fas_eval_dataset.tfrecords'
...
t1 = time.time()
# Loop each dir
# TODO: need to modify the structure of dataset directory
'''
...
for idx, line in enumerate(gt_content):
    ...
    # for i in range(len(cur_hand_joints_x)):
    #     cv2.circle(cur_img, center=(int(cur_hand_joints_x[i]), int(cur_hand_joints_y[i])), radius=3, color=(0, 0, 255), thickness=-1)
    #     # cv2.putText(cur_img, str(i), (int(cur_hand_joints_x[i]), int(cur_hand_joints_y[i])), cv2.FONT_HERSHEY_PLAIN, 2.0, (0, 0, 255), 2)
    #     cv2.imshow('', cur_img)
    #     # cv2.waitKey(200)
    # cv2.imshow('', cur_img)
...
# no error before this line
    ...
    if cur_img.shape[0] > cur_img.shape[1]:
        ...
        output_image[:, int(box_size / 2 - math.floor(image.shape[1] / 2)): int(
            box_size / 2 + math.floor(image.shape[1] / 2) + offset), :] = image
        cur_hand_joints_x = list(map(lambda x: x + (box_size / 2 - math.floor(image.shape[1] / 2)),
                                cur_hand_joints_x))
